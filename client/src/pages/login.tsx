...
export default function LoginPage({ onLoggedIn }: { onLoggedIn: () => void }) {
  ...
  if (!supabase) {
    return (
      <div className="stripe-page min-h-screen flex flex-col items-center justify-center px-4">
  ...
  return (
    <div className="stripe-page min-h-screen px-4 py-10">
      <div className="stripe-shell mx-auto flex min-h-[calc(100vh-5rem)] max-w-5xl items-center justify-center">
        <div className="grid w-full items-center gap-10 lg:grid-cols-[1.05fr_420px]">
          <div className="hidden lg:block">
            <div className="stripe-kicker mb-6">Accesso</div>
            <h1 className="stripe-display text-[clamp(3rem,4vw,5rem)]">
              Bilanci, benchmark e memo AI
              <span className="bg-[linear-gradient(120deg,#635bff_0%,#0a84ff_60%,#ff7a00_100%)] bg-clip-text text-transparent"> in un'unica vista.</span>
            </h1>
            <p className="stripe-lead mt-6 max-w-xl">
              Un'interfaccia piu' chiara per leggere la cassa, confrontare il mercato e trasformare numeri grezzi in decisioni.
            </p>
          </div>
          ...
          <Card className="w-full max-w-sm lg:max-w-none">
            <CardHeader className="pb-4">
              <div className="mb-3 flex items-center justify-center gap-3">
                <BilancioLogo className="w-10 h-10 text-primary" />
                <span className="text-2xl font-semibold tracking-[-0.04em]">BilancioAI</span>
              </div>
      ...
      <div className="stripe-shell mt-8 w-full"><PoweredByAttribution /></div>
    </div>
  );
}
